...
All collections share the same metadata schema (see above documentation). It is possible to submit a request to change the labels of the metadata for your repository. To change the display label for your repository's collection(s) please [email Collections U of T ITS staff](mailto:digitalinitiatives@library.utoronto.ca) to request modifications per metadata element and the label you would like to have displayed. ITS will consult with you and, if approved, modify [internal metadata mapping documents](https://git.library.utoronto.ca/utl-its/digital-collections/collections-api/-/tree/feature_merged/src/configs/metadataMapping). Note that these internal metadata display documents also control the labels for your collection items' IIIF manifests.
